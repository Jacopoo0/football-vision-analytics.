...
def build_panel(stats, sec, team_counts, frame_idx, max_frames) -> np.ndarray:
    ...
    _t(d, "VELOCITA'", (M+14, y+8), 9, _MUTED, bold=True)
    _hline(d, M+14, W-M-14, y+22, _BORDER)
...
def build_canvas(frame_img, panel_img, minimap_img=None):
    ...
    # Minimap sovrapposta in basso a sinistra sul frame
...
class InferenceWorker(threading.Thread):
    ...
    def run(self):
        while not self._stop:
            ...
            try:
                # Calibra/aggiorna omografia
                self.homography.update_frame(frame)

                # Rilevazione giocatori e arbitri
                ...
                # Filtra detection troppo piccole
                if len(player_det) > 0:
                    areas = (
...
def main():
    ...
    while True:
        ...
        if frame_idx % 60 == 0:
            ...
            print(
                f"[{frame_idx}/{max_frames}] {sec:.1f}s | "
                f"Poss {p0:.0f}/{p1:.0f}% | "
                f"Pass {ps0}/{ps1} | "
